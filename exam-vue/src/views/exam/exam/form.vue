    ...
    <el-card style="margin-top: 20px">
      ...
      <div>
        ...
        >
          ...
          <!--            <template v-slot="scope">-->
          <!--&lt;!&ndash;              / {{ scope.row.totalMulti }}&ndash;&gt;-->
          ...
          <!--            <template v-slot="scope">-->
          <!--&lt;!&ndash;              / {{ scope.row.totalJudge }}&ndash;&gt;-->
...
<!--                            {{ scope.row.totalJudge }}-->
              <el-input-number v-model="scope.row.operCount" :min="0" :max="scope.row.totalOper" :controls="false" style="width: 100px" /> / {{ scope.row.totalOper }}
            </template>
          ...
          >
            <template v-slot="scope">
              <el-input-number v-model="scope.row.operScore" :min="0" :controls="false" style="width: 100%" />
            </template>
          </el-table-column>
    ...
    <el-card style="margin-top: 20px">
      ...
      <el-form ref="postForm" :model="postForm" :rules="rules" label-position="left" label-width="120px">
        ...
        <el-form-item label="是否限时" required>
          <el-checkbox v-model="postForm.timeLimit" checked disabled/>
        </el-form-item>

        <el-form-item v-if="postForm.timeLimit" disabled label="考试时间" prop="totalTime">
    ...
    <el-card style="margin-top: 20px;">
      ...
      <el-radio-group v-model="postForm.openType" style="margin-bottom: 20px">
        ...
        <el-radio :label="2" border>考试场次</el-radio>
      </el-radio-group>
...
export default {
    ...
    repoList: {
      ...
      handler() {
        ...
        for (let i = 0; i<this.repoList.length; i++) {
          ...
          this.postForm.totalScore += item.operCount * item.operScore
  ...
  methods: {
    ...
    // 添加子项
    handleAdd() {
      this.repoList.push({ id: '', rowId: new Date().getTime(), radioCount: 0, radioScore: 0, multiCount: 0, multiScore: 0, judgeCount: 0, judgeScore: 0, operCount: 0, operScore: 0, saqCount: 0, saqScore: 0 })
    },
    ...
    fetchData(id) {
      fetchDetail(id).then(response => {
        ...
        this.postForm.timeLimit = this.postForm.timeLimit == 1
        ...
        if (this.postForm.depList.length > 0) {
          this.postForm.departIds = []
          this.postForm.depList.forEach(item => {
            this.postForm.departIds.push(item.departId)
          })
        }
